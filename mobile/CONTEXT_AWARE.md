...
Apply `supabase/010_quest_date_validation.sql` after migration 009. Creation and editing reject invalid calendar dates, past scheduled starts/deadlines, and deadlines before the scheduled start. Inputs use device-local time with minute precision (the current minute is allowed). Preferred daily times remain unrestricted. Server validation checks newly assigned dates against server time; existing overdue quests remain completable and historical dates are not rewritten.
