...
export default function HomeScreen() {
  const { user, profile, activeQuests, completedQuests, isLoading, isRefreshing, error, levelProgress, refresh } = useUserData();
  const displayName = profile?.display_name || user?.email?.split('@')[0] || (user ? 'Explorer' : 'Guest');
  const { ranked } = useQuestPriority();
  const recommendations = ranked.filter(item => item.tier !== 'history').slice(0, 2);
        ...
        <ScrollView
          ...
          <View style={styles.header}>
            ...
            <View style={styles.copy}>
              <ThemedText style={styles.eyebrow}>YOUR DAILY OVERVIEW</ThemedText>
            ...
            <>
              ...
              <View style={styles.card}>
                <View style={styles.sectionHeading}>
                  ...
                  <ThemedText style={styles.count}>{activeQuests.length} active</ThemedText>
                </View>
                {recommendations.length ? recommendations.map(({ quest, reasons, tier }) => (
                  <View key={quest.id} style={styles.questPreview}>
                    <View style={styles.questIcon}><Ionicons name="sparkles-outline" size={22} color={COLORS.honeyDeep} /></View>
                    <View style={styles.copy}>
                      <ThemedText style={styles.eyebrow}>{tier === 'now' ? 'RECOMMENDED NOW' : tier === 'next' ? 'UP NEXT' : 'TO CONSIDER'}</ThemedText>
                      <ThemedText style={styles.questTitle} numberOfLines={2}>{quest.title}</ThemedText>
                      <ThemedText style={styles.body} numberOfLines={2}>{reasons.slice(0, 2).join(' Â· ')}</ThemedText>
                    </View>
                  </View>
                )) : (
                ...
                <OverviewAction label="View all quests" onPress={() => router.push('/quests')} />
              </View>
              ...
              <TouchableOpacity style={styles.growthCard} accessibilityRole="button" accessibilityLabel="View skill tree" onPress={() => router.push('/skill-tree')} activeOpacity={0.8}>
                ...
                <View style={styles.copy}>
                  ...
                  <ThemedText style={styles.body}>{completedQuests.length > 0 ? `${completedQuests.length} quest${completedQuests.length === 1 ? '' : 's'} completed. See your growth.` : 'See the strengths you can grow.'}</ThemedText>
                </View>
...
function OverviewAction({ label, onPress, onDark = false }: { label: string; onPress: () => void; onDark?: boolean }) {
  return (
    <TouchableOpacity style={[styles.action, onDark && styles.actionOnDark]} onPress={onPress} accessibilityRole="button" activeOpacity={0.8}>
      <ThemedText style={[styles.actionText, onDark && styles.actionTextOnDark]}>{label}</ThemedText>
      <Ionicons name="arrow-forward" size={18} color={onDark ? COLORS.honey : COLORS.ink} />
    </TouchableOpacity>
  );
}
...
const styles = StyleSheet.create({
  ...
  safeArea: { flex: 1 },
  content: { width: '100%', maxWidth: MaxContentWidth, alignSelf: 'center', paddingHorizontal: 20, paddingTop: 20, paddingBottom: 112, gap: 20 },
  ...
  progressCard: { backgroundColor: COLORS.honeyDeep, borderRadius: Radii.xl, padding: 22, gap: 12, ...BeeBetterShadow },
  onDarkMuted: { color: COLORS.honeySoft },
  levelTitle: { fontSize: 28, lineHeight: 35, fontWeight: '800', color: COLORS.card, marginTop: 4 },
  ...
  progressText: { color: COLORS.honeySoft, fontSize: 13, lineHeight: 20 },
  progressTrack: { height: 7, backgroundColor: '#896448', borderRadius: Radii.pill, overflow: 'hidden' },
  ...
  questPreview: { flexDirection: 'row', alignItems: 'center', gap: 12 },
  ...
  questTitle: { fontSize: 16, lineHeight: 23, fontWeight: '700', color: COLORS.ink, marginVertical: 3 },
  ...
  actionOnDark: { borderTopColor: '#896448', marginTop: 4 },
  actionText: { flex: 1, fontSize: 13, lineHeight: 20, fontWeight: '800', color: COLORS.ink },
  actionTextOnDark: { color: COLORS.honey },
