...
export function QuestTimeInput({ label, value, onChange, timeOnly = false }: QuestTimeInputProps) {
  ...
  return <View style={{ gap: 8 }}>
    ...
    {picker && <>
      <DateTimePicker value={picker.value} mode={picker.mode} display={Platform.OS === 'ios' ? 'spinner' : 'default'} accentColor={C.honeyDark} onDismiss={() => setPicker(null)} onValueChange={(_, selected) => {
